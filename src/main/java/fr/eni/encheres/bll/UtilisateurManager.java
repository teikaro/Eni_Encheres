...
public class UtilisateurManager {
	...
	public void checkInsertUtilisteur(Utilisateur user) throws ClassNotFoundException, BusinessException {
		...
		if (user.getPseudo().length() < 30 && user.getNom().length() < 30 &&
		  ...
		  } else { be = new BusinessException();
		  be.ajouterErreur(CodesResultatBLL.INFOS_KO); }
		 
		/*
		 * checkPseudo(user.getPseudo(), be); checkMail(user.getEmail(), be);
		 * checkNom(user.getNom(), be); checkPrenom(user.getPrenom(), be);
		 * checkNumber(user.getTelephone(), be); checkRue(user.getRue(), be);
		 * checkVille(user.getVille(), be); checkCP(user.getCode_postal(), be);
		 * checkMdp(user.getMot_de_passe(), be);
		 * 
		 * if(!be.hasErreurs()) { daoUser.inserUtilisateur(user); }else { throw be; }
		 */
	}

	private void checkPseudo(String pseudo, BusinessException be) throws BusinessException {
		if(pseudo.matches(STRING_VERIFICATION) || pseudo.length() > 30) {
			be.ajouterErreur(CodesResultatBLL.PSEUDO_KO);
		}else {
			Utilisateur test = daoUser.getUserByPseudo(pseudo);
			if (test.getPseudo() == pseudo) {
	...
	private void checkNumber(String number, BusinessException be) throws BusinessException {
		if(number.matches(CHECK_NUMBER) || number.length() > 30) {
			be.ajouterErreur(CodesResultatBLL.PHONE_KO);
		}
	}
	
	private void checkRue(String number, BusinessException be) throws BusinessException {
		if(number.length() > 30) {
			be.ajouterErreur(CodesResultatBLL.RUE_KO);
		}
	}
	
	private void checkCP(String number, BusinessException be) throws BusinessException {
		if(number.matches(CHECK_NUMBER) || number.length() > 10) {
